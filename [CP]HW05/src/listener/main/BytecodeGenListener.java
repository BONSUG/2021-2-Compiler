...
public class BytecodeGenListener extends MiniCBaseListener implements ParseTreeListener {
	...
	public void exitStmt(MiniCParser.StmtContext ctx) {
		...
		if(ctx.getChildCount() > 0)
		{
			...
			// <(0) Fill here>
			else if(ctx.if_stmt() !=null)  //if문
				stmt += newTexts.get(ctx.if_stmt());
			else if(ctx.while_stmt() !=null)  //while 문
				stmt += newTexts.get(ctx.while_stmt());
			else if(ctx.return_stmt() !=null) {//리턴문
	...
	public void exitWhile_stmt(MiniCParser.While_stmtContext ctx) {
			// <(1) Fill here!>
		String stmt = "";
		String cond = newTexts.get(ctx.expr());//조건
		String cond_stmt = newTexts.get(ctx.stmt()); 
		String Lloop = symbolTable.newLabel(); //루프 시작
		String Lend = symbolTable.newLabel(); //종료
	...
	public void exitFun_decl(MiniCParser.Fun_declContext ctx) {
			// <(2) Fill here!>
		String t1= funcHeader(ctx, getFunName(ctx)); //함수 정의 도입문
		String compound = newTexts.get(ctx.compound_stmt())+"\n"; //함수 내부 문장
		t1+=compound;
		int i = ctx.compound_stmt().stmt().size();
		
		 if (ctx.compound_stmt().stmt(i- 1).return_stmt() == null) {
			 //리턴문 확인
	         t1 += "return\n"; //main문 관련
		 }
	    newTexts.put(ctx, t1 + ".end method\n");
	...
	public void exitCompound_stmt(MiniCParser.Compound_stmtContext ctx) {
		...
		for(Local_declContext i : ctx.local_decl()) {
			stmt += newTexts.get(i); // local_ decl 수 만큼 추가
		}
		for(StmtContext i : ctx.stmt()) {
			stmt += newTexts.get(i); //stmt 수 만큼 추가
		}
		newTexts.put(ctx, stmt);		
	}
	...
	public void exitReturn_stmt(MiniCParser.Return_stmtContext ctx) {
		...
		if(i==1) 
			stmt=ctx.RETURN().toString()+"\n";
		else {
			String expr = newTexts.get(ctx.expr());
			stmt+=expr+"i"+ctx.RETURN().toString()+"\n";//정수 리턴
		}
		newTexts.put(ctx, stmt);
